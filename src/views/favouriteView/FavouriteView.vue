  ...
  <div class="favourite">
    <div class="container">
      ...
      <div class="favourite-div">
        <h1 class="favourite__cards-title" v-if="favouriteStore.favourites.length > 0">{{ $t('favourite__cards-title') }}
        </h1>
        <div class="favourite__cards">
          <div class="favourite__card" v-for="item in favouriteStore.favourites" :key="item.id">
            ...
            <div class="favourite__card__theme">
              ...
              <p class="favourite__card__theme-nation">{{ calculateFormattedPrice(item.price) }} $/{{
                $t('card__theme-nation') }}</p>
              <p class="favourite__card__theme-price">{{ $t('card__theme-price') }} {{ item.price }} $</p>
              <p class="favourite__card__theme-newprice">{{ $t('card__theme-newprice') }} {{ calculateFormatted(item) }} $
              </p>
              ...
              <div class="favourite__card__icons">
                ...
                <button class="favourite__card__icons-btnb"
                  @click="toggleBasket(item.id, item.title, item.thumbnail, $t('fixed__theme-title-2'))">
