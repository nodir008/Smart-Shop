...
const calculateFormatted = () => {
    const discountedPrice = productSingleStore.product?.price - (productSingleStore.product?.price / 100) * productSingleStore.product?.discountPercentage;
...
const updateTotalPrice = () => {
    const discountedPrice = productSingleStore.product?.price * productQuantity.value - totalFormattedPrice.value;
    return discountedPrice.toFixed(0);
};

const handleHeartClick = () => {
    favouriteStore.activeStates[productSingleStore.product?.id] = !favouriteStore.activeStates[productSingleStore.product?.id];
...
const addToOrder = (productId) => {
    if (productId === productSingleStore.product?.id) {
        ...
        const deliveryTimeFormatted = `${deliveryTime.getHours().toString().padStart(2, "0")}:${deliveryTime.getMinutes().toString().padStart(2, "0")}:${deliveryTime
            .getSeconds()
            .toString()
            .padStart(2, "0")} ${deliveryTime.getDate().toString().padStart(2, "0")}-${(deliveryTime.getMonth() + 1).toString().padStart(2, "0")}-${deliveryTime.getFullYear()}`;
...
<template>
    <section class="product">
        <div class="container">
            <div class="product__images">
                <swiper @swiper="setThumbsSwiper" :loop="true" :grabCursor="true" :spaceBetween="1" :slidesPerView="3" :freeMode="true" :watchSlidesProgress="true" :modules="modules" class="mySwiper">
                    <swiper-slide imgper-slide v-for="(image, i) in productSingleStore.product?.images" :key="i"><img class="swiper__slide-img" :src="image" alt="" /></swiper-slide>
                </swiper>
                <swiper
                    :style="{
                    ...
                    class="mySwiper2"
                >
                    <swiper-slide v-for="(image, i) in productSingleStore.product?.images" :key="i"><img class="swiper__slide-img2" :src="image" alt="" /></swiper-slide>
                </swiper>
            </div>
            ...
            <div class="product__theme">
                <div class="product__theme-rating">
                    <div class="product__theme-rating-text">
                        <p class="product__theme-rating-rating">
                            <StarIcon />{{ productSingleStore.product?.rating }} ( <span class="product__theme-rating-rating-s"> {{ $t("product__theme-rating-rating-s") }} </span> )
                        </p>
                        <p class="product__theme-rating-stock">{{ productSingleStore.product?.stock }} {{ $t("product__theme-rating-stock") }}</p>
                    </div>
                    <!-- @click="toggleFavorite" :class="{ active: isFavourite }" -->
                    <button class="product__theme-rating-favourite" @click="handleHeartClick">
                        <HeartIcon class="product__theme-rating-favourite-headericon-1" v-if="!favouriteStore.activeStates[productSingleStore.product?.id]" />
                        <Heart2Icon class="product__theme-rating-favourite-headericon-2" v-else />
                        {{ favouriteStore.activeStates[productSingleStore.product?.id] ? $t("product__theme-rating-favourite-headericon-2") : $t("product__theme-rating-favourite-headericon-1") }}
                    </button>
                </div>
                ...
                <div class="product__theme-count">
                    <div class="product__theme-count-number">
                        <button class="product__theme-count-number-b" @click="decreaseQuantity">
                            <MinusIcon class="product__theme-count-number-b-icon1" :class="{ active: productQuantity === 1 }" />
                        </button>
                ...
                <div class="product__theme-price">
                    ...
                    <p class="product__theme-price-price2">{{ productSingleStore.product?.price * productQuantity }}$</p>
                    <p class="product__theme-price-price3">{{ productQuantity }} {{ $t("product__theme-price-price3-1") }}-{{ updateTotalPrice() }}$ {{ $t("product__theme-price-price3-2") }}</p>
                ...
                <div class="product__theme-term" @click="toggleLoader">
                    <div class="product__theme-term-div">
                        <p class="product__theme-term-p">
                            ...
                            {{ calculateFormattedPrice(productSingleStore.product?.price * productQuantity) }}$
                        </p>
            ...
            <div class="product__theme-none">
                ...
                <div class="product__theme-none-basket">
                    ...
                    <button class="product__theme-none-basket-btns-btn" @click="addToBasket2">
                        {{ $t("product__theme-btns-btn") }}
                    </button>
                </div>
    ...
    <div class="product__nation-div" :class="{ active: show }" @click="show = false">
        <div class="product__nation" @click.stop="">
            ...
            <ul class="product__nation-ul">
                <li class="product__nation-ul-li"><CheckIcon class="product__nation-ul-li-icon" /> {{ $t("product__nation-ul-li-1") }}</li>
                <li class="product__nation-ul-li"><CheckIcon class="product__nation-ul-li-icon" /> {{ $t("product__nation-ul-li-2") }}</li>
                <li class="product__nation-ul-li"><CheckIcon class="product__nation-ul-li-icon" /> {{ $t("product__nation-ul-li-3") }}</li>
            </ul>
            <RouterLink to="/installment" class="product__nation-btn">{{ $t("product__nation-btn") }}</RouterLink>
        </div>
    </div>

    <button class="product__nation-bg-2" :class="{ active: show }" @click="show = false"></button>
